# ============================================================
# server/app.py — Telemetry Decoder Demo API
# ============================================================
# Thin FastAPI wrapper around the EXISTING pipeline. Does NOT
# reimplement any DSP/ML/FEC — it imports demo_journey and
# demo_v8_5_fec and calls their functions.
#
# Run:
#   cd pipeline                 # so the modules find each other + checkpoint
#   uvicorn ../server/app:app --host 0.0.0.0 --port 8000 --reload
# or set PIPELINE_DIR env var to the pipeline folder (see below).
#
# Endpoints:
#   GET  /api/health   -> {status, device}
#   POST /api/run      -> full signal-journey + pre/post-FEC result
# ============================================================
...
# the pipeline's internal _load() resolves sibling files relative to cwd /
# /content; make cwd the pipeline dir so those lookups succeed.
os.chdir(PIPELINE)
